...
public interface ConnectInterceptor<S> {
	...
	/**
	 * Called during connection initiation, immediately before user
	 * authorization. May be used to store custom connection attributes in the
	 * session before redirecting the user to the provider's site or to
	 * contribute parameters to the authorization URL.
	 */
	void preConnect(ConnectionFactory<S> connectionFactory, MultiValueMap<String, String> parameters, WebRequest request);

	/**
	 * Called immediately after the connection is established. Used to invoke
	 * the service API on behalf of the user upon connecting.
	 */
