...
public class SolrClient {
	 ...
	 */
	public static <T> PaginationSupport<T> query(String keyword, Class<T> cls,
			int start, int rows, SolrServer server) {
		...
		int totalCount = Long.valueOf(response.getResults().getNumFound())
				.intValue();
		return new PaginationSupport<T>(EntityConvert.solrDocument2Entity(sdl,
				cls), totalCount, start, rows);
	}
	 ...
	 * @return PaginationSupport<T>
	 */
	public static <T> PaginationSupport<T> query(SolrParams params,
			Class<T> cls, int start, int rows, SolrServer server) {
		QueryResponse response = null;
		try {
	 ...
	 * @param server
	 */
	public static void update(Map<Object, Object> objMap, String idName,
			SolrServer server) {
		if (objMap != null && objMap.size() > 0
			...
			try {
				...
				if (sdl.size() > 0) {
					...
					updateRequest.add(EntityConvert
							.solrDocumentList2SolrInputDocumentList(sdl,
									idName, objMap));
	...
	public static void update(Object obj, String idName, SolrServer server) {
		if (obj != null && StringUtils.isNotBlank(idName)) {
			...
			try {
				Method method = cls.getMethod(EntityConvert.getMethodName(
						idName, "get"));
				...
				if (o != null) {// 主键不能为空
					...
					updateRequest.setAction(
							AbstractUpdateRequest.ACTION.COMMIT, false, false);
					updateRequest.add(EntityConvert
							.solrDocument2SolrInputDocument(sd, obj));
					updateRequest.process(server);
	 ...
	 * @param server
	 */
	public static void deleteByExample(Object obj, String idName,
			SolrServer server) {
		Class<?> cls = obj.getClass();
		try {
			Method method = cls.getMethod(EntityConvert.getMethodName(idName,
					"get"));
			Object o = method.invoke(obj);
			if (o != null) {
	 ...
	 * @param server
	 */
	public static void deleteById(Object[] idArrays, String idName,
			SolrServer server) {
		if (idArrays.length > 0) {
			try {
				StringBuffer query = new StringBuffer(idName + ":"
						+ idArrays[0]);
	...
	public static void optimize(SolrServer server) {
		try {
			...
			logger.info("优化 SOLR SYSTEM 完毕花费时间"
