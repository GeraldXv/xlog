...
<div class="Settings">
	<ul>
		<li><a id="ssource" href="addsources.html">SOURCES</a> <a
			href="account.html">ACCOUNT</a> <a href="Profile.html">&nbsp;PROFILE&nbsp;</a>
		</li>
...
<div id="ssource1" class="ssource">
	<ul>
		<li>Add Source<br />
			<ul>
				...
				<c:forEach var="providerId" items="${providerIds}">
					<li><form name="${providerId}form" method="POST"
							action="<c:url value="/connect/${providerId}" />">
							...
							<c:if test="${providerId=='facebook'}">
								<input type="hidden" name="scope"
									value="publish_stream,user_photos,offline_access,read_stream" />
							</c:if>
							<c:set var="connections" value="${connectionMap[providerId]}" />
							<s:message code="${providerId}.displayName"
								var="providerDisplayName" />
							<s:message code="${providerId}.png" var="iconUrl" />
							<c:if test="${empty connections}">
								<img src="<c:url value="${iconUrl}" />" />
								<br />
								<a href="javascript:document.${providerId}form.submit()"> <img
									src="<c:url value="/resources/image/addbutton.png" />" />
								</a>
							</c:if>
						</form></li>
				</c:forEach>
				...
				<c:if test="${isGmailconnected==false}">
					...
					<li><img src="<c:url value="${iconUrl}" />" /><br /> <a
						href="#" onclick="popaut()"><img
		...
		<li>Delete Source<br />
			<ul>
				<c:forEach var="providerId" items="${providerIds}">
					<li><form name="${providerId}formdelete" method="POST"
							action="<c:url value="/connect/${providerId}" />">
							...
							<c:if test="${not empty connections}">
								<img src="<c:url value="${iconUrl}" />" />
								<br />
								<a href="javascript:document.${providerId}formdelete.submit()">
									<img src="<c:url value="/resources/image/delbutton.png" />" />
								</a>
							</c:if>
						</form></li>
				</c:forEach>
