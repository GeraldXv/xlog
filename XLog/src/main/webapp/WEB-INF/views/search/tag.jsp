...
<div class="SRcontent">
				...
				<tr>
					<td>
						<li class="spl"></li> <c:if test="${range=='none' }">
							...
							<li><a id="t1" href="<c:url value="/search/status?query=${query }&page=1&range=none" />" onclick="timchg('t1')">Anytime</a></li>
						</c:if> <c:if test="${range=='hour' }">
							<li><a id="t2" href="<c:url value="/search/status?query=${query }&page=1&range=hour" />" class="seled" onclick="timchg('t2')">Past
									hour</a></li>
						</c:if> <c:if test="${range!='hour' }">
							...
							<li><a id="t3" href="<c:url value="/search/status?query=${query }&page=1&range=day" />" class="seled" onclick="timchg('t3')">Past
									24 hour</a></li>
						</c:if> <c:if test="${range!='day' }">
							...
							<li><a id="t4" href="<c:url value="/search/status?query=${query }&page=1&range=week" />" class="seled" onclick="timchg('t4')">Past
									week</a></li>
						</c:if> <c:if test="${range!='week' }">
							...
							<li><a id="t5" href="<c:url value="/search/status?query=${query }&page=1&range=month" />" class="seled" onclick="timchg('t5')">Past
									month</a></li>
						</c:if> <c:if test="${range!='month' }">
							...
							<li><a id="t6" href="<c:url value="/search/status?query=${query }&page=1&range=year" />" class="seled" onclick="timchg('t6')">Past
									year</a></li>
						</c:if> <c:if test="${range!='year' }">
							<li><a id="t6" href="<c:url value="/search/status?query=${query }&page=1&range=year" />" onclick="timchg('t6')">Past year</a></li>
				...
				<li class="reslt">
					<table>
						<c:forEach var="status" items="${statusList}">
							<tr class="person">
								<td rowspan="2" width="50"><span><c:if test="${not empty status.userImage}">
	...
	<ul class="pages">
		<li><a id="prep" href="<c:url value="/search/status?query=${query}&page=${currentPage-1}&range=none" />">Prev&lt;</a> <c:forEach
				var="i" begin="1" end="${totalPage }" step="1" varStatus="status">
				<c:if test="${i==currentPage }">
					<a><c:out value="${i}" /> </a>
				</c:if>
				<c:if test="${i!=currentPage }">
					<a href="<c:url value="/search/status?query=${query}&page=${i}&range=none" />"><c:out value="${i}" /> </a>
				</c:if>
			</c:forEach> <a id="nxtp" href="<c:url value="/search/status?query=${query}&page=${currentPage+1}&range=none" />">&gt;Next</a></li>
	</ul>
</div>
